...
namespace Barnacle.Models
{
    public static class CameraUtils
    {
        ...
        public static List<Point> Convert3DPoints(List<Point3D> points3d, Viewport3D vp)
        {
            // Get the combined transformation matrix.
            Viewport3DVisual visual =
                VisualTreeHelper.GetParent(vp.Children[0]) as Viewport3DVisual;
            ...
            List<Point3D> copiedPoints = new List<Point3D>();
            foreach (Point3D p in points3d)
            {
                copiedPoints.Add(matrix.Transform(p));
            }
            // Convert the Point3Ds into Points.
            List<Point> points2d = new List<Point>();
            for (int i = 0; i < numPoints; i++)
            {
                points2d.Add(new Point(copiedPoints[i].X, copiedPoints[i].Y));
            }
            return points2d;
        }
