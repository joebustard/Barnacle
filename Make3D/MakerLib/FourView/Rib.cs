...
namespace MakerLib.FourView
{
    internal class Rib
    {
        public static List<System.Windows.Point> GenerateProfilePoints(int ribDivisions, List<System.Windows.Point> sourcePnts)
        {
            List<System.Windows.Point> profilePoints = new List<System.Windows.Point>();
            ...
            if (sourcePnts != null && sourcePnts.Count > 0)
            {
                // if necessary close the polygon
                if ((sourcePnts[0].X != sourcePnts[sourcePnts.Count - 1].X) ||
                     (sourcePnts[0].Y != sourcePnts[sourcePnts.Count - 1].Y))
                {
                    sourcePnts.Add(new Point(sourcePnts[0].X, sourcePnts[0].Y));
                }
                ...
                {
                    if (i < sourcePnts.Count - 1)
                    {
                        pathLength += Distance(sourcePnts[i], sourcePnts[i + 1]);
                    }
                    if (sourcePnts[i].X < tlx)
                    {
                        tlx = sourcePnts[i].X;
                    }

                    if (sourcePnts[i].Y < tly)
                    {
                        tly = sourcePnts[i].Y;
                    }

                    if (sourcePnts[i].X > brx)
                    {
                        brx = sourcePnts[i].X;
                    }

                    if (sourcePnts[i].Y > bry)
                    {
                        bry = sourcePnts[i].Y;
                    }
                }
                ...
                for (int div = 0; div < ribDivisions; div++)
                {
                    ...
                    if (targetDistance < pathLength)
                    {
                        ...
                        while (!found && cp < sourcePnts.Count)
                        {
                            Point p0 = sourcePnts[cp - 1];
                            Point p1 = sourcePnts[cp];
                                 ...
                                 (runningDistance + d >= targetDistance))
                            {
                                ...
                                if (overhang != 0.0)
                                {
                                    ...
                                    profilePoints.Add(new Point(nx, ny));
                                }
                                else
                                {
                                    double nx = p0.X;
                ...
                /*
                 ...
                 for (int i = 0; i < profilePoints.Count; i++)
                 {
                     ...
                     if ((profilePoints[i].X > cx) && (profilePoints[j].X < cx))
                     {
                         ...
                         Point BottomPoint = new Point(cx, profilePoints[i].Y - dy);
                     ...
                     if ((profilePoints[i].X < cx) && (profilePoints[j].X > cx))
                     {
                         ...
                         Point TopPoint = new Point(cx, profilePoints[i].Y + dy);
