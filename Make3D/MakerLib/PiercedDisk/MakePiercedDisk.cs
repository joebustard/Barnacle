...
namespace MakerLib
{
    public class PiercedDiskMaker : MakerBase
    {
        ...
        public void Generate(Point3DCollection pnts, Int32Collection faces)
        {
            ...
            double hrad = diskRadius - holeRadius - holeDistanceFromEdge;
            ...
            {
                double d = Distance(outerPoly[i], rim[rim.Count - 1]);
                if (d < closeDistance)
                {
                    closest = i;
                    closeDistance = d;
            ...
            foreach (System.Windows.Point p in innerPoly)
            {
                pf.Add(new System.Drawing.PointF((float)p.X, (float)p.Y));
            }
        ...
        public void SetValues(double diskRadius,
                              double holeRadius,
                              double holeDistanceFromEdge,
                              double numberOfHoles,
                              double diskHeight)
        {
            this.diskRadius = diskRadius;
            this.holeRadius = holeRadius;
            this.holeDistanceFromEdge = holeDistanceFromEdge;
        ...
        {
            paramLimits.AddLimit("DiskRadius", 1, 200);
            paramLimits.AddLimit("HoleRadius", 0.1, 200);
            paramLimits.AddLimit("HoleDistanceFromEdge", 0.1, 200);
            paramLimits.AddLimit("NumberOfHoles", 1, 200);
