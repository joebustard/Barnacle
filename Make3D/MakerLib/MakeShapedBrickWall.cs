...
namespace MakerLib
{
    public class ShapedBrickWallMaker : MakerBase
    {
        ...
        public ShapedBrickWallMaker(string path, double brickLength, double brickHeight, double brickDepth, double mortarGap)
        {
            this.path = path;
        ...
        public void Generate(Point3DCollection pnts, Int32Collection faces)
        {
            ...
            // do the back
            TriangulateSurface(tmp.ToArray(), -brickDepth,true);

            // sides
            for (int i = 0; i < tmp.Count; i++)
            {
                int j = (i + 1) % tmp.Count;
                int v0 = AddVertice(tmp[i].X, tmp[i].Y, -brickDepth);
                int v1 = AddVertice(tmp[j].X, tmp[j].Y, -brickDepth);
